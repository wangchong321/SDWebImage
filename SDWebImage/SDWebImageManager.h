 ...
 * 是异步下载器 (SDWebImageDownloader) 和图像缓存存储 (SDImageCache) 之间的纽带
 ...
 * "缓存过滤器"是一个 block，用于每次 SDWebImageManager 需要将一个 URL 转换成一个缓存的键值，可用于删除图像 URL 中动态的部分
 *
 * The following example sets a filter in the application delegate that will remove any query-string from the
 * URL before to use it as a cache key:
 * 以下示例在 application 代理中设置一个过滤器，该过滤器会在将 URL 当作缓存键值之前，从 URL 中删除请求字符串
 *
 * @code
